...
async function filtrarOrcamentos(status) {
    ["todos", "Pendente", "Aprovado"].forEach((s) => {
        ...
        if (!btn) return;
        if (s.toLowerCase() === status.toLowerCase() || (status === "todos" && s === "todos")) {
            btn.className = "px-4 py-2 text-sm font-medium bg-emerald-600 text-white rounded-lg shadow-sm transition";
        } else {
            btn.className = "px-4 py-2 text-sm font-medium bg-white text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 transition";
        }
    });

    if (!window.supabaseClient) return;
    try {
        let query = window.supabaseClient.from("orcamentos").select("*").order("created_at", { ascending: false }).limit(50);
...
function carregarTabelaFiltrada(lista) {
    ...
    tbody.innerHTML = lista.map((orc) => {
        ...
        if (statusAtual === "Aprovado") badgeClass = "bg-green-100 text-green-800";
        if (statusAtual === "Agendado") badgeClass = "bg-purple-100 text-purple-800";
...
function adicionarItemOrcamento() {
    if (!servicoSelecionado) {
        return alert("Por favor, pesquise e clique em um serviço da lista antes de adicionar.");
    }
    const quantidade = parseInt(document.getElementById("orc-qtd").value) || 1;
    const valorUnit = parseFloat(document.getElementById("orc-valor-unit").value) || 0;
...
async function salvarOrcamentoDinamico(event) {
    ...
    const valorTotal = parseFloat(document.getElementById("orc-valor-total-geral").value) || 0;

    if (itensOrcamentoAtual.length === 0) return alert("Adicione serviços ao orçamento.");

    const resumoServicos = itensOrcamentoAtual.map((i) => `${i.quantidade}x ${i.servico}`).join(", ");
...
async function aprovarOrcamento(id) {
    ...
    try {
        ...
        const clientId = await processarClienteBanco(orc.nome_lead || orc.nome || "Cliente sem nome", orc.whatsapp || "");
...
async function deletarOrcamento(id) {
    ...
    try {
        const { error } = await window.supabaseClient.from("orcamentos").delete().eq("id", id);
        if (error) throw error;
        carregarOrcamentos();
    } catch (error) {
...
async function enviarOrcamentoWhatsAppDinamico(id, dados = null) {
    let orc = dados;
    if (!orc) {
        const { data, error } = await window.supabaseClient.from("orcamentos").select("*").eq("id", id);
    ...
    if (numLimpo && !numLimpo.startsWith("55") && !numLimpo.startsWith("54")) numLimpo = "55" + numLimpo;
    ...
    window.open(`https://wa.me/${numLimpo}?text=${encodeURIComponent(textoMsg)}`, "_blank");
}

async function gerarPDFOrcamentoDinamico(orcamentoId) {
    if (!window.supabaseClient) return;
    try {
        const { data, error } = await window.supabaseClient.from("orcamentos").select("*").eq("id", orçamentoId);
        if (error || !data || data.length === 0) throw new Error("Orçamento não encontrado.");
        const orc = data[0];
...
function executarAcaoWhatsApp() {
    if (window.orcamentoAtualSalvo) enviarOrcamentoWhatsAppDinamico(window.orcamentoAtualSalvo.id, window.orcamentoAtualSalvo);
    fecharModalPosSalvamento();
}

function executarAcaoPDF() {
    if (window.orcamentoAtualSalvo) gerarPDFOrcamentoDinamico(window.orcamentoAtualSalvo.id);
    fecharModalPosSalvamento();
}