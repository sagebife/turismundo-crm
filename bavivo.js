    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            listaAvisosBA = [{
                ...
                tempo: "Hoje",
                mensagem: "Bem-vindo ao CRM Turismundo na nuvem! Adicione seus avisos.",
...
function iniciarTickerBA() {
    ...
    if (listaAvisosBA.length === 0) {
        atualizarCardVisualBA({ categoria: "Sem Avisos", tempo: "-", mensagem: "Nenhum aviso cadastrado.", link: "#", cor: "azul" });
        return;
    }
...
function atualizarCardVisualBA(aviso) {
    ...
    if(linkEl) linkEl.setAttribute("href", aviso.link && aviso.link !== "#" ? aviso.link : "https://linda.buenosaires.gob.ar/");
...
function renderizarListaAvisosBA() {
    ...
    container.innerHTML = listaAvisosBA.map(aviso => `
...
function mostrarFormularioBA(idEditar) {
    ...
    if (idEditar && idEditar !== 'undefined') {
        ...
        if (aviso) {
            ...
            document.getElementById("input-ba-categoria").value = aviso.categoria || "";
            ...
            document.getElementById("input-ba-link").value = aviso.link === "#" ? "" : (aviso.link || "");
        }
    } else {
...
async function salvarAvisoBA() {
    ...
    const dadosAviso = {
        categoria: document.getElementById("input-ba-categoria").value.trim() || "Aviso",
        cor: document.getElementById("input-ba-cor").value,
        tempo: document.getElementById("input-ba-tempo").value.trim() || "Agora",
        mensagem: document.getElementById("input-ba-mensagem").value.trim() || "Sem detalhes.",
